...
if __name__ == "__main__":
    ...
    model_path = os.path.join(script_dir, "weights/last.pt")
    model = YOLO(model_path)

    if len(sys.argv) > 1:
        video_path = sys.argv[1]
    else:
        video_path = os.path.join(script_dir, "test_videos/CityUtdR.mp4")
        print(f"No video path provided. Using default video: {video_path}")
